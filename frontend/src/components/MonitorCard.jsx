...
const MonitorCard = ({ monitor, onDelete }) => {
  ...
  // --- LOGIC BARU: Determine Effective Status ---
  // Mengutamakan response time jika status bukan 'down'
  const getEffectiveStatus = (currentStatus, time) => {
    if (currentStatus === "down") return "down"; // Jika mati, tetap mati
    
    // Logika Slow: antara 1000ms dan 5000ms
    if (time >= 1000 && time <= 5000) {
      return "slow";
    }
    
    // Jika diatas 5000 biasanya dianggap down/timeout, 
    // tapi disini kita kembalikan ke status asli jika > 5000 atau < 1000
    return currentStatus; 
  };

  const displayStatus = getEffectiveStatus(monitor.status, monitor.response_time);

  // Mengambil styling berdasarkan status yang sudah dihitung
  const {
    bgColor,
    textColor,
    badgeColor,
  ...
  // --- LOGIC BARU: Update warna Text Response Time ---
  const getResponseTimeColor = (time) => {
    if (time < 1000) return "text-emerald-500 font-black"; // Cepat (< 1s)
    if (time <= 5000) return "text-amber-500 font-black"; // Lambat (1s - 5s)
    return "text-red-500 font-black"; // Sangat lambat/Timeout (> 5s)
  };
  ...
  return (
    <Card
      className={`group relative hover:shadow-2xl transition-all duration-500 rounded-[2.5rem] border-slate-100 overflow-hidden bg-white ${
    ...
    >
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant="secondary"
                  className="bg-slate-100 text-slate-400 border-none rounded-full text-[9px] font-black px-2 py-0.5"
                >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="rounded-full w-10 h-10 p-0 hover:bg-slate-50"
                >
              ...
              <DropdownMenuContent
                align="end"
                className="w-52 rounded-2xl p-2 shadow-xl border-slate-100"
              >
                <DropdownMenuItem
                  onClick={() => handleDetailMonitor(monitor.id)}
                ...
                >
                  {monitor.paused ? (
                    ...
                    <>
                      <Pause className="w-4 h-4 mr-2" /> Pause Monitoring
                    </>
                  )}
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  ...
                  className="rounded-xl cursor-pointer text-red-500 focus:text-red-500 focus:bg-red-50"
                >
      ...
      <CardContent className="p-7 pt-4 space-y-6">
        ...
        <div
          className={`flex items-center justify-between p-4 rounded-[1.5rem] border ${bgColor} transition-all duration-500`}
        >
          ...
          <Badge
            className={`${badgeColor} text-white border-0 rounded-full px-3 py-0.5 text-[10px] font-black tracking-widest`}
          >
            {/* Tampilkan displayStatus agar sesuai dengan logika Slow */}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            <span className="font-bold text-slate-400 uppercase text-[11px] tracking-wider">
              Uptime Percentage
            </span>
            <span
              className={`font-black text-lg ${
                uptimePercentage < 90 ? "text-orange-500" : "text-blue-600"
              }`}
            >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <p
              className={`text-3xl font-black ${getResponseTimeColor(
                monitor.response_time
              )}`}
            >
              {monitor.response_time}
              <span className="text-sm ml-1 font-bold opacity-30">ms</span>
            </p>
          </div>
          ...
          <div className="text-center border-r border-slate-100 px-1">
            ...
            <p className="text-sm font-black text-blue-500 mt-1">
              {responseTimeChartData.length > 0
                ...
                : monitor.response_time}{" "}
              <span className="text-[9px] opacity-60 font-medium">ms</span>
            </p>
          </div>
          <div className="text-center border-r border-slate-100 px-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">
        ...
        <div className="pt-2">
          <p className="mb-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">
            24-Hour Performance
          </p>
          <div className="h-[140px] w-full relative">
            ...
            {uptimeChartData.length > 0 && (
              <div className="flex justify-between mt-2 px-1 text-[9px] font-black text-slate-300 uppercase tracking-tighter">
                ...
                <span>
                  {uptimeChartData[Math.floor(uptimeChartData.length / 2)]?.time}
                </span>
                <span>
                  {uptimeChartData[uptimeChartData.length - 1].time}
                </span>
              </div>
            )}
          </div>
